...
OUT_DIRECTORY = "/home/victor/newLITSreducedblankstest"
...
def ndarry2jpg_png((data_file, img_gt_file, out_dir)):
    ...
    for i in xrange(0, num_slices):
        if np.array_equal(
            ...
            print i, num_empty_to_keep, data_file, img_gt_file, spacing, 'fail_empty!'
            continue
...
def convert(data_dir, out_dir):
    ...
    list_test = p.map(ndarry2jpg_png, zip(vols, segs, itertools.repeat(out_dir, len(vols))))
    p.close()
