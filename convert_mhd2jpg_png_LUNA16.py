...
def ndarry2jpg_png(data_file, out_dir, subsetIndex, flist):
    ...
    for i in xrange(0, img.shape[2]):
        ...
        try:
            scipy.misc.imsave(
                os.path.join(out_dir + "JPEGImages/subset" + str(subsetIndex), fn + "_" + str(i) + ".jpg"), img3c)
        except ValueError:
            print fn
            pass
        # im = Image.fromarray(img3c)
        # im.save(os.path.join(out_dir+"JPEGImages/subset"+str(subsetIndex),fn+"_"+str(i)+"_"+".jpg"))
        cv2.imwrite(os.path.join(out_dir + "PNGImages/subset" + str(subsetIndex), fn + "_" + str(i) + ".png"),
                    img_gt[:, :, i])
        flist.write(
            "/JPEGImages/subset" + str(subsetIndex) + "/" + fn + "_" + str(i) + ".jpg " + "/PNGImages/subset" + str(
                subsetIndex) + "/" + fn + "_" + str(i) + ".png\n")
...
def convert(data_dir, out_dir):
    ...
    for i in xrange(0, 10):
        ...
        if not os.path.exists(out_dir + "JPEGImages/subset" + str(i)):
            os.makedirs(out_dir + "JPEGImages/subset" + str(i))
        if not os.path.exists(out_dir + "PNGImages/subset" + str(i)):
            os.makedirs(out_dir + "PNGImages/subset" + str(i))
