...
def main():
    ...
    restore_var = [v for v in tf.global_variables() if 'conv1' not in v.name or not args.first_run]
    ...
    for i in xrange(0, args.num_classes):
        ...
        loss.append(
            softmax_weights_per_class[i] * tf.losses.sparse_softmax_cross_entropy(logits=prediction, labels=gt,
                                                                                  weights=tf.where(
    ...
    saver = tf.train.Saver(var_list=tf.global_variables(), max_to_keep=10, restore_sequentially=True)
